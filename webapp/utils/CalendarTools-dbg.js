...
ZLEAVE_REQ_CREATE.utils.CalendarTools = (function() {
	...
	return {
		...
		getDayLabelsForMonth : function(oDate, successCallback, errorCallback) {
			...
			var oRequestedMonthStartDate = ZLEAVE_REQ_CREATE.utils.CalendarTools.calcMonthStartDate(oDate); // use the 1st day of the month as key
			var oNextMonthStartDate = ZLEAVE_REQ_CREATE.utils.CalendarTools.calcNextMonthStartDate(oRequestedMonthStartDate);
			var oPreviousMonthStartDate = ZLEAVE_REQ_CREATE.utils.CalendarTools.calcPreviousMonthStartDate(oRequestedMonthStartDate);
	
			// check if work schedule results are already in the cache for 3 months
			var oCachedResult = ZLEAVE_REQ_CREATE.utils.CalendarTools.oCache[oRequestedMonthStartDate];
			var oCachedResultPrevious = ZLEAVE_REQ_CREATE.utils.CalendarTools.oCache[oPreviousMonthStartDate];
			var oCachedResultNext =ZLEAVE_REQ_CREATE.utils.CalendarTools.oCache[oNextMonthStartDate];
			...
			if(oCachedResult === undefined){
				oEndDate = ZLEAVE_REQ_CREATE.utils.CalendarTools.approximateMonthEndDate(oNextMonthStartDate);
			}
			//if previous month doesn't exist
			else if(oCachedResultPrevious === undefined){
				oEndDate = ZLEAVE_REQ_CREATE.utils.CalendarTools.approximateMonthEndDate(oPreviousMonthStartDate);
			...
			//if next month doesn't exist
				...
				oEndDate = ZLEAVE_REQ_CREATE.utils.CalendarTools.approximateMonthEndDate(oEndDate);
				oRequestedMonthStartDate = new Date(oStartDate.getFullYear(), oStartDate.getMonth() + 1, 1);
			}
			...
			ZLEAVE_REQ_CREATE.utils.DataManager.getLeaveRequestsForTimePeriod(oStartDate, oEndDate, function(
					aLeaveRequests) {
				if (_alreadyFailed) {
					return;
				}
				...
				if (_aWorkSchedules !== null) {
					ZLEAVE_REQ_CREATE.utils.CalendarTools._finish(oStartDate, oEndDate, oRequestedMonthStartDate,
			...
			ZLEAVE_REQ_CREATE.utils.DataManager.getWorkSchedulesForTimePeriod(oStartDate, oEndDate, function(
					aWorkSchedules) {
				if (_alreadyFailed) {
					return;
				}
		...
		_calcDayLabelsForMonth : function(_aLeaveRequests, _aWorkSchedules, oMonthStartDate, iDayOffset) {
			...
			var oResult = {};
			var iMonthDayCount = ZLEAVE_REQ_CREATE.utils.CalendarTools.calcMonthDayCount(oMonthStartDate);
			for ( var iDay = 0; iDay < iMonthDayCount; iDay++) {
				...
				for ( var iLeave = 0; iLeave < _aLeaveRequests.length; iLeave++) {
					if (_aLeaveRequests[iLeave].StatusCode
						...
						var oStart = ZLEAVE_REQ_CREATE.utils.Formatters.getDate(_aLeaveRequests[iLeave].StartDate);
						var oEnd = ZLEAVE_REQ_CREATE.utils.Formatters.getDate(_aLeaveRequests[iLeave].EndDate);
						...
						if (ZLEAVE_REQ_CREATE.utils.CalendarTools.dayRangeMatch(oDayDate, oStart, oEnd)) {
		...
		_finish : function(oStartDate, oEndDate, oRequestedMonthStartDate, oPreviousMonthStartDate, oNextMonthStartDate,
			...
			try {
				...
				if (oStartDate < oRequestedMonthStartDate) {
					ZLEAVE_REQ_CREATE.utils.CalendarTools.oCache[oPreviousMonthStartDate] = this._calcDayLabelsForMonth(_aLeaveRequests, _aWorkSchedules, oPreviousMonthStartDate, 0);
					iDayOffset += ZLEAVE_REQ_CREATE.utils.CalendarTools.calcMonthDayCount(oPreviousMonthStartDate);
				}
				oDayLabels = ZLEAVE_REQ_CREATE.utils.CalendarTools.oCache[oRequestedMonthStartDate] = this._calcDayLabelsForMonth(
				    _aLeaveRequests, _aWorkSchedules, oRequestedMonthStartDate, iDayOffset);
				iDayOffset += ZLEAVE_REQ_CREATE.utils.CalendarTools.calcMonthDayCount(oRequestedMonthStartDate);
				var oRequestedEndDate = ZLEAVE_REQ_CREATE.utils.CalendarTools.approximateMonthEndDate(oRequestedMonthStartDate);
				if (oEndDate > oRequestedEndDate) {
					ZLEAVE_REQ_CREATE.utils.CalendarTools.oCache[oNextMonthStartDate] = this._calcDayLabelsForMonth(
							_aLeaveRequests, _aWorkSchedules, oNextMonthStartDate, iDayOffset);
				}
				//include all three months
				var oDayLabelsPrev = ZLEAVE_REQ_CREATE.utils.CalendarTools.oCache[oPreviousMonthStartDate];
				var oDayLabelsNext = ZLEAVE_REQ_CREATE.utils.CalendarTools.oCache[oNextMonthStartDate];
