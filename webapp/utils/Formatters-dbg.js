...
ZLEAVE_REQ_CREATE.utils.Formatters = (function() {
	return {
		...
		DATE_ODATA_MMMyyyy: function(oValue) {
			...
			var oDate = ZLEAVE_REQ_CREATE.utils.Formatters.getDate(oValue);
			if (oDate !== null) {
		...
		BALANCE: function(oValue) {
			...
			return ZLEAVE_REQ_CREATE.utils.Formatters
				.adjustSeparator(ZLEAVE_REQ_CREATE.utils.Formatters
		...
		DURATION: function(sDays, sHours) {
			...
			sDays = ZLEAVE_REQ_CREATE.utils.Formatters
				.stripDecimals(sDays);
			if (sDays) {
				var pos = sDays.indexOf(".");
				if (pos < 0)
					return ZLEAVE_REQ_CREATE.utils.Formatters.adjustSeparator(sDays);
			}
			return ZLEAVE_REQ_CREATE.utils.Formatters.DURATION_FORMAT(ZLEAVE_REQ_CREATE.utils.Formatters.stripDecimals(sHours));
		},
		...
		DURATION_UNIT: function(sDays, sHours) {
			...
			if (sDays) {
				var pos = sDays.indexOf(".");
				if (pos < 0)
					return (sDays * 1 !== 1) ? ZLEAVE_REQ_CREATE.utils.Formatters.resourceBundle
						.getText("LR_DAYS") : ZLEAVE_REQ_CREATE.utils.Formatters.resourceBundle
						.getText("LR_DAY");
			}
			return (sHours * 1 !== 1) ? ZLEAVE_REQ_CREATE.utils.Formatters.resourceBundle
				.getText("LR_HOURS") : ZLEAVE_REQ_CREATE.utils.Formatters.resourceBundle.getText("LR_HOUR");
		},
		...
		FORMAT_DATETIME: function(sPrefix, oValue) {
			...
			return sPrefix + " " + ZLEAVE_REQ_CREATE.utils.Formatters
		...
		FORMATTER_INTRO: function(aRelatedRequests) {
			...
			if (sLeaveRequestType && sLeaveRequestType.toString() === "2") {
				if (sStatusCode === "SENT") {
					return ZLEAVE_REQ_CREATE.utils.Formatters.resourceBundle
		...
		FORMAT_ENDDATE: function(sHyphen, sWorkingDaysDuration, sStartTime,
			...
			try {
				if (sHyphen && sWorkingDaysDuration && sStartTime && sEndDate && sEndTime) {
					if (ZLEAVE_REQ_CREATE.utils.Formatters
						.isHalfDayLeave(sWorkingDaysDuration)) {
						return ZLEAVE_REQ_CREATE.utils.Formatters
							.TIME_hhmm(sStartTime) + " " + sHyphen + " " + ZLEAVE_REQ_CREATE.utils.Formatters
							.TIME_hhmm(sEndTime);
					} else if (sWorkingDaysDuration * 1 !== 1) {
						return sHyphen + " " + ZLEAVE_REQ_CREATE.utils.Formatters.DATE_ODATA_EEEdMMMyyyy(sEndDate);
					}
				}
			} catch (e) {
				// ignore
			...
			try {
				if (sHyphen && sWorkingDaysDuration && sStartTime && sEndDate && sEndTime) {
					if (ZLEAVE_REQ_CREATE.utils.Formatters
						...
						return sHyphen + " " + ZLEAVE_REQ_CREATE.utils.Formatters
		...
		FORMAT_RELATED_START_DATE: function(aRelatedRequests) {
			if (aRelatedRequests !== undefined && aRelatedRequests.length > 0 && aRelatedRequests[0].LeaveRequestType == "2" && aRelatedRequests[
					0].StartDate !=
				undefined) {
				try {
					return ZLEAVE_REQ_CREATE.utils.Formatters
		...
		SET_RELATED_END_DATE_VISIBILITY: function(aRelatedRequests) {
			return aRelatedRequests != undefined && aRelatedRequests.length > 0 && aRelatedRequests[0].LeaveRequestType == "2" &&
				...
				aRelatedRequests[0].EndTime != undefined && (ZLEAVE_REQ_CREATE.utils.Formatters
		...
		FORMAT_RELATED_END_DATE: function(sHyphen, aRelatedRequests) {
			if (aRelatedRequests != undefined && aRelatedRequests.length > 0 && aRelatedRequests[0].LeaveRequestType == "2" && aRelatedRequests[0]
				.WorkingDaysDuration !=
				...
				try {
					if (ZLEAVE_REQ_CREATE.utils.Formatters
						...
						return ZLEAVE_REQ_CREATE.utils.Formatters
							.TIME_hhmm(aRelatedRequests[0].StartTime) + " " + sHyphen + " " + ZLEAVE_REQ_CREATE.utils.Formatters
		...
		calculateUsed: function(BalanceUsedQuantity, BalanceApprovedQuantity, BalanceRequestedQuantity) {
			...
			sBalanceTotalUsedQuantity = ZLEAVE_REQ_CREATE.utils.Formatters.BALANCE(sBalanceTotalUsedQuantity.toString());
		...
		_parseNotes: function(notesString) {
			try {
				...
				for (i = 1; i < notesArray.length; i++) {
					...
					d = temp[3].toString();
					t = temp[4].toString();
					oDate = new Date(d.substring(0, 4), parseInt(d.substring(4, 6), 10) - 1, d.substring(6, 8), t.substring(0, 2), t.substring(2, 4), t
						.substring(4, 6));
		...
		_parseAttachments: function(attachmentString, RequestID, oModel) {
			try {
				...
				var oEntry = {};
				var temp, i, d, t, oDate;
				var oPernr = ZLEAVE_REQ_CREATE.utils.UIHelper.getPernr();
				...
				for (i = 1; i < attachments.length; i++) {
					...
					oDate = new Date(d.substring(0, 4), parseInt(d.substring(4, 6), 10) - 1, d.substring(6, 8), t.substring(0, 2), t.substring(2, 4), t
						.substring(4, 6));
					// if user is behind GMT we convert the dates to UTC FA 2419918 <<				
					if (new Date().getTimezoneOffset() > 0) {
						oDate = this.toUTCDate(oDate);
					} // FA 2419918 >>
					var oDateFormat = sap.ca.ui.model.format.DateFormat.getDateTimeInstance({
						style: 'medium'
					});
					...
					if (temp[5]) {
						...
						oEntry.FileUrl = oModel.sServiceUrl + "/FileAttachmentSet(EmployeeID='" + oPernr + "',LeaveRequestId='" + RequestID +
							"',ArchivDocId='" + temp[5] + "',FileName='" + encodeURIComponent(oEntry.FileName) + "')/$value";
						// Note 2299607 <<
					} else {
						jQuery.sap.log.error("ArchivDocId is missing for LeaveRequestID:" + RequestID, [], [
							"ZLEAVE_REQ_CREATE.utils.Formatters._parseAttachments"
						]);
						oEntry.FileUrl = "";
					}
					result.push(oEntry);
		...
		formatterAbsenceDuration: function(WorkingDaysDuration, WorkingHoursDuration, Alldf) {
			...
			if (Alldf) {
				finalduration = ZLEAVE_REQ_CREATE.utils.Formatters.stripDecimals(WorkingDaysDuration);
			} else {
				finalduration = ZLEAVE_REQ_CREATE.utils.Formatters.stripDecimals(WorkingHoursDuration);
			}
		...
		formatterAbsenceDurationUnit: function(WorkingDaysDuration, WorkingHoursDuration, Alldf) {
			...
			if (Alldf) {
				if (parseInt(WorkingDaysDuration, 10) === 1) {
					finalUnit = ZLEAVE_REQ_CREATE.utils.Formatters.resourceBundle.getText("LR_DAY");
				} else {
					finalUnit = ZLEAVE_REQ_CREATE.utils.Formatters.resourceBundle.getText("LR_DAYS");
				}
			} else {
				if (parseInt(WorkingHoursDuration, 10) === 1) {
					finalUnit = ZLEAVE_REQ_CREATE.utils.Formatters.resourceBundle.getText("LR_HOUR");
				} else {
					finalUnit = ZLEAVE_REQ_CREATE.utils.Formatters.resourceBundle.getText("LR_HOURS");
				}
			}
			return finalUnit;
		}
