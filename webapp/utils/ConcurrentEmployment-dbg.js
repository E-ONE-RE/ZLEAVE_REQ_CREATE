...
ZLEAVE_REQ_CREATE.utils.ConcurrentEmployment = {
	...
	getCEEnablement: function(self, successHandler) {
		...
		ZLEAVE_REQ_CREATE.utils.DataManager.getPersonellAssignments(self, function(data) {
			if (data.length > 1) {
				oModel.setData(data);
				...
				ZLEAVE_REQ_CREATE.utils.UIHelper.setPernr(data[0].Pernr);
				successHandler();
			...
			buttons: [
			    new sap.m.Button({
					...
					press: function() {
						...
						ZLEAVE_REQ_CREATE.utils.UIHelper.setPernr(self.oApplication.pernr);
						successHandler();
						if (that.secondSuccessHandler) {
							that.secondSuccessHandler();
							that.secondSuccessHandler = null;
