...
// FA 2469113>>
		getApprover: function(successCallback, errorCallback) {
			...
			this._getOData(sPath, null, arrParams, function(objResponse) {
				...
				errorCallback(ZLEAVE_REQ_CREATE.utils.DataManager.parseErrorMessages(objResponse));
			});
...
//FA 2360245 to expose all the all the properties of the collection configuration to the odata
			if (!_cachedModelObj.DefaultConfigurations) {
//FA 2360245 <<
				//this._getOData(sPath, null, arrParams, function(objResponse) {
				this._getOData(sPath, null, [], function(objResponse) {
//FA 2360245 >>
					var oConfiguration;
					try {
						var oResult = objResponse.results;
						...
						if (oConfiguration === undefined) {
							deferredDefaultType.reject(ZLEAVE_REQ_CREATE.utils.DataManager.parseErrorMessages(objResponse));
							return;
						}
					} catch (e) {
						deferredDefaultType.reject(ZLEAVE_REQ_CREATE.utils.DataManager.parseErrorMessages(e));
						return;
					}
		...
		getAbsenceTypeCollection: function() {
			...
			if (!_cachedModelObj.AbsenceTypeCollection) {
				this._getOData(sPath, null, oParams, function(objResponse) {
					...
					deferredAbsTypeColl.reject(ZLEAVE_REQ_CREATE.utils.DataManager.parseErrorMessages(objResponse));
				});
			} else {
		...
		getBalancesForAbsenceType: function(sAbsenceTypeCode, sAttabsInd, successCallback, errorCallback) {   //HEGDEPR 2381879 Begin Change
            var oPernr = ZLEAVE_REQ_CREATE.utils.UIHelper.getPernr();
            var sPath;
            if(sAttabsInd == ""){
            	sPath = "AbsenceTypeCollection(EmployeeID='" + oPernr + "',AbsenceTypeCode='" + sAbsenceTypeCode +
            	"')/absenceTypeTimeAccount";
            } else {
				sPath = "AbsenceTypeCollection(EmployeeID='" + oPernr + "',AbsenceTypeCode='" + sAbsenceTypeCode +
				"',AttabsInd='" + sAttabsInd + "')/absenceTypeTimeAccount";
			}																								 //HEGDEPR 2381879 End Change
			...
			this._getOData(sPath, null, arrParams, function(objResponse) {
				...
				try {
					...
					if (oResult instanceof Array && oResult.length > 0) {
						...
						sBalancePlanned = ZLEAVE_REQ_CREATE.utils.Formatters.BALANCE(iBalancePlanned.toString());
						sBalanceAvailable = ZLEAVE_REQ_CREATE.utils.Formatters.BALANCE(iBalanceAvailable.toString());
						sBalanceUsed = ZLEAVE_REQ_CREATE.utils.Formatters.BALANCE(iBalanceUsed.toString());
						sBalanceTotalUsedQuantity = ZLEAVE_REQ_CREATE.utils.Formatters.BALANCE((iBalanceUsed + iBalancePlanned).toString());
					}
				} catch (e) {
		...
		submitLeaveRequest: function(oNewEntry, bProcessCheckOnlyInd, successCallback, errorCallback, uploadFileAttachments) {
			...
			this._postOData("LeaveRequestCollection", oNewEntry, function(objResponseData, objResponse) {
				...
				if(!objResponseData.RequestID){
				    ...
				    ZLEAVE_REQ_CREATE.utils.UIHelper.errorDialog(msg);
				}else{
				...
				errorCallback(ZLEAVE_REQ_CREATE.utils.DataManager.parseErrorMessages(objResponseData));
			});
		},
		...
		getLeaveRequestsForTimePeriod: function(oStartDate, oEndDate, successCallback, errorCallback) {
			...
			var sStartDate = ZLEAVE_REQ_CREATE.utils.Formatters.DATE_YYYYMMdd(oStartDate) + 'T00:00:00';
			var sEndDate = ZLEAVE_REQ_CREATE.utils.Formatters.DATE_YYYYMMdd(oEndDate) + 'T00:00:00';
			var sPath = "LeaveRequestCollection";
            var oPernr = ZLEAVE_REQ_CREATE.utils.UIHelper.getPernr();
...
//END OF NOTE 2306536									
        ...
        searchApprover: function(searchString, successCallback){
			...
			this._getOData(sPath, null, arrParams, function(objResponse) {
				try {
					...
					ZLEAVE_REQ_CREATE.utils.DataManager.parseErrorMessages([_resourceBundle.getText("LR_DD_PARSE_ERR") + " (DataManager.getApprover)"]);
					 sap.ca.ui.utils.busydialog.releaseBusyDialog();
					return;
				}
			}, function(objResponse) {
			    sap.ca.ui.utils.busydialog.releaseBusyDialog();
			    ZLEAVE_REQ_CREATE.utils.DataManager.parseErrorMessages(objResponse);
			});
        },
        getPersonellAssignments : function(appController, fSuccess) {
                        this._getOData("/ConcurrentEmploymentSet", null, [], function(oData) {
								...
								function(oError) {
									ZLEAVE_REQ_CREATE.utils.DataManager.parseErrorMessages(oError);
								});
		},
...
//BEGIN OF NOTE 2404328
        LeaveRequestValueHelp: function(fieldName, successCallback, errorCallback){
        	var sPath = "LeaveRequestHelpCollection?";
        	var oUrlParams = "$filter=FIELDNAME eq '"+fieldName+"'";
        	_modelBase.read(sPath,null,oUrlParams,true,function(response){
        		successCallback(response);
        	},function(response){
        		errorCallback(response);
        	});
